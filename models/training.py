"""
Model training system for VAE pipeline - FIXED VERSION
"""

import os
import torch
import torch.optim as optim
from torch.utils.data import DataLoader, TensorDataset
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
import pickle
from sklearn.model_selection import train_test_split
import time
from datetime import timedelta
from typing import Dict, List, Tuple, Optional, Any
import logging

from .vae import VAE, VAELoss, BetaScheduler, create_model
from .callbacks import EarlyStopping, ModelCheckpoint, CallbackList

logger = logging.getLogger(__name__)

class ModelTrainer:
    """
    Handles training of VAE models with different configurations.
    """
    
    def __init__(self, config):
        """
        Initialize trainer with configuration.
        
        Args:
            config: Configuration object
        """
        self.config = config
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        logger.info(f"Using device: {self.device}")
        
        # Set random seeds for reproducibility
        self._set_random_seeds()
        
        # Load data
        self.data_loader = self._setup_data_loaders()
        
    def _set_random_seeds(self, seed: int = 42) -> None:
        """Set random seeds for reproducibility."""
        np.random.seed(seed)
        torch.manual_seed(seed)
        if torch.cuda.is_available():
            torch.cuda.manual_seed_all(seed)
    
    def _setup_data_loaders(self) -> Tuple[DataLoader, DataLoader]:
        """Setup training and validation data loaders."""
        # Load preprocessed data
        df_prepared = pd.read_csv(self.config.paths.PREPROCESSED_FILE)
        
        # Load preprocessing objects for metadata
        with open(os.path.join(self.config.paths.DATA_DIR, 'preprocessing_objects.pkl'), 'rb') as f:
            self.preprocessing_objects = pickle.load(f)
        
        # Create train/validation split
        X_train, X_val = train_test_split(
            df_prepared.values, 
            test_size=0.2, 
            random_state=42
        )
        
        # Convert to tensors
        X_train_tensor = torch.FloatTensor(X_train).to(self.device)
        X_val_tensor = torch.FloatTensor(X_val).to(self.device)
        
        # Create datasets and loaders
        train_dataset = TensorDataset(X_train_tensor, X_train_tensor)
        val_dataset = TensorDataset(X_val_tensor, X_val_tensor)
        
        train_loader = DataLoader(
            train_dataset, 
            batch_size=self.config.model.BATCH_SIZE, 
            shuffle=True
        )
        val_loader = DataLoader(
            val_dataset, 
            batch_size=self.config.model.BATCH_SIZE, 
            shuffle=False
        )
        
        logger.info(f"Training samples: {len(X_train)}, Validation samples: {len(X_val)}")
        
        return train_loader, val_loader
    
    def train_single_model(
        self, 
        strategy: str, 
        beta: float,
        save_dir: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        Train a single VAE model with specified parameters.
        
        Args:
            strategy: Annealing strategy
            beta: Beta value for KL weighting
            save_dir: Directory to save model and results
            
        Returns:
            Dictionary with training results
        """
        logger.info(f"Training model: strategy={strategy}, beta={beta}")
        
        if save_dir is None:
            save_dir = os.path.join(
                self.config.paths.MODELS_DIR, 
                strategy, 
                f'beta_{beta}'
            )
        
        os.makedirs(save_dir, exist_ok=True)
        
        # Create model
        model = self._create_model()
        
        # Setup training components
        optimizer = optim.Adam(model.parameters(), lr=self.config.model.LEARNING_RATE)
        loss_fn = VAELoss(
            self.preprocessing_objects['categorical_cardinality'],
            len(self.preprocessing_objects['num_cols'])
        )
        
        # Get beta schedule
        beta_schedule = BetaScheduler.get_schedule(
            beta_start=0.0,
            beta_end=beta,
            num_epochs=self.config.model.NUM_EPOCHS,
            strategy=strategy
        )
        
        # Setup callbacks
        callbacks = self._setup_callbacks(save_dir)
        
        # Training loop
        training_results = self._training_loop(
            model, optimizer, loss_fn, beta_schedule, callbacks, save_dir
        )
        
        # Save final model
        self._save_model(model, optimizer, training_results, save_dir, strategy, beta)
        
        return training_results
    
    def train_all_models(self) -> Dict[str, Dict[str, Any]]:
        """
        Train models for all strategy-beta combinations.
        
        Returns:
            Dictionary with results for all trained models
        """
        all_results = {}
        
        total_models = len(self.config.training.ANNEALING_STRATEGIES) * len(self.config.training.BETA_VALUES)
        current_model = 0
        
        for strategy in self.config.training.ANNEALING_STRATEGIES:
            strategy_results = {}
            
            for beta in self.config.training.BETA_VALUES:
                current_model += 1
                logger.info(f"\n🧠 Training model {current_model}/{total_models}")
                
                try:
                    results = self.train_single_model(strategy, beta)
                    strategy_results[beta] = results
                    
                except Exception as e:
                    logger.error(f"Failed to train model {strategy}-{beta}: {e}")
                    strategy_results[beta] = {'error': str(e)}
            
            all_results[strategy] = strategy_results
            
            # Create strategy-level summary
            self._create_strategy_summary(strategy, strategy_results)
        
        # Create cross-strategy comparisons
        self._create_cross_strategy_comparisons(all_results)
        
        return all_results
    
    def _create_model(self) -> VAE:
        """Create VAE model instance."""
        return create_model(
            input_dim=len(self.data_loader[0].dataset[0][0]),
            num_numerical=len(self.preprocessing_objects['num_cols']),
            cat_dict=self.preprocessing_objects['categorical_cardinality'],
            hidden_dim=self.config.model.HIDDEN_DIM,
            latent_dim=self.config.model.LATENT_DIM
        ).to(self.device)
    
    def _setup_callbacks(self, save_dir: str) -> CallbackList:
        """Setup training callbacks."""
        callbacks = []
        
        # Early stopping
        if self.config.training.EARLY_STOPPING:
            early_stopping = EarlyStopping(
                monitor='val_loss',
                min_delta=self.config.training.EARLY_STOPPING_MIN_DELTA,
                patience=self.config.training.EARLY_STOPPING_PATIENCE,
                verbose=True,
                mode='min',
                restore_best_weights=self.config.training.RESTORE_BEST_WEIGHTS
            )
            callbacks.append(early_stopping)
        
        # Model checkpoint
        model_checkpoint = ModelCheckpoint(
            filepath=os.path.join(save_dir, 'checkpoint_epoch_{epoch:03d}_val_loss_{val_loss:.4f}.pth'),
            monitor=self.config.training.CHECKPOINT_MONITOR,
            verbose=True,
            save_best_only=self.config.training.SAVE_BEST_ONLY,
            mode=self.config.training.CHECKPOINT_MODE
        )
        callbacks.append(model_checkpoint)
        
        return CallbackList(callbacks)
    
    def _training_loop(
        self,
        model: VAE,
        optimizer: torch.optim.Optimizer,
        loss_fn: VAELoss,
        beta_schedule: List[float],
        callbacks: CallbackList,
        save_dir: str
    ) -> Dict[str, Any]:
        """Main training loop."""
        train_loader, val_loader = self.data_loader
        
        # Initialize tracking
        history = {
            'train_loss': [],
            'val_loss': [],
            'num_loss': [],
            'cat_loss': [],
            'kl_loss': [],
            'beta_values': []
        }
        
        callbacks.set_model(model)
        callbacks.on_train_begin()
        
        start_time = time.time()
        
        for epoch in range(self.config.model.NUM_EPOCHS):
            current_beta = beta_schedule[epoch]
            
            callbacks.on_epoch_begin(epoch)
            
            # Training phase
            train_metrics = self._train_epoch(model, train_loader, optimizer, loss_fn, current_beta)
            
            # Validation phase
            val_metrics = self._validate_epoch(model, val_loader, loss_fn, current_beta)
            
            # Update history
            history['train_loss'].append(train_metrics['total_loss'])
            history['val_loss'].append(val_metrics['total_loss'])
            history['num_loss'].append(train_metrics['num_loss'])
            history['cat_loss'].append(train_metrics['cat_loss'])
            history['kl_loss'].append(train_metrics['kl_loss'])
            history['beta_values'].append(current_beta)
            
            # Logging
            if (epoch + 1) % 10 == 0 or epoch == self.config.model.NUM_EPOCHS - 1:
                logger.info(
                    f'Epoch {epoch+1}/{self.config.model.NUM_EPOCHS}: '
                    f'Train Loss: {train_metrics["total_loss"]:.4f}, '
                    f'Val Loss: {val_metrics["total_loss"]:.4f}, '
                    f'Beta: {current_beta:.2f}'
                )
            
            # FIXED: Create proper logs dictionary with correct val_loss key
            logs = {
                # Training metrics
                'loss': train_metrics['total_loss'],
                'num_loss': train_metrics['num_loss'],
                'cat_loss': train_metrics['cat_loss'],
                'kl_loss': train_metrics['kl_loss'],
                # Validation metrics
                'val_loss': val_metrics['total_loss'],  # FIXED: Direct mapping
                'val_num_loss': val_metrics['num_loss'],
                'val_cat_loss': val_metrics['cat_loss'],
                'val_kl_loss': val_metrics['kl_loss'],
                # Additional info
                'beta': current_beta
            }
            
            callbacks.on_epoch_end(epoch, logs)
            
            # Check for early stopping
            if callbacks.get_stop_training():
                logger.info(f"Early stopping triggered at epoch {epoch + 1}")
                break
        
        callbacks.on_train_end()
        
        training_time = time.time() - start_time
        
        # Save training history
        self._save_training_history(history, save_dir)
        
        # Create training plots
        self._create_training_plots(history, save_dir)
        
        return {
            'history': history,
            'training_time': training_time,
            'epochs_trained': epoch + 1,
            'final_metrics': {
                'train_loss': history['train_loss'][-1],
                'val_loss': history['val_loss'][-1],
                'num_loss': history['num_loss'][-1],
                'cat_loss': history['cat_loss'][-1],
                'kl_loss': history['kl_loss'][-1]
            }
        }
    
    def _train_epoch(
        self,
        model: VAE,
        train_loader: DataLoader,
        optimizer: torch.optim.Optimizer,
        loss_fn: VAELoss,
        beta: float
    ) -> Dict[str, float]:
        """Train for one epoch."""
        model.train()
        total_loss = 0
        num_loss = 0
        cat_loss = 0
        kl_loss = 0
        
        for batch_idx, (data, _) in enumerate(train_loader):
            data = data.to(self.device)
            optimizer.zero_grad()
            
            # Forward pass
            decoded_outputs, mu, logvar = model(data)
            
            # Compute loss
            loss, n_loss, c_loss, k_loss = loss_fn(decoded_outputs, data, mu, logvar, beta)
            
            # Backward pass
            loss.backward()
            optimizer.step()
            
            # Accumulate losses
            total_loss += loss.item()
            num_loss += n_loss.item()
            cat_loss += c_loss.item()
            kl_loss += k_loss.item()
        
        # Average losses
        n_samples = len(train_loader.dataset)
        return {
            'total_loss': total_loss / n_samples,
            'num_loss': num_loss / n_samples,
            'cat_loss': cat_loss / n_samples,
            'kl_loss': kl_loss / n_samples
        }
    
    def _validate_epoch(
        self,
        model: VAE,
        val_loader: DataLoader,
        loss_fn: VAELoss,
        beta: float
    ) -> Dict[str, float]:
        """Validate for one epoch."""
        model.eval()
        total_loss = 0
        num_loss = 0
        cat_loss = 0
        kl_loss = 0
        
        with torch.no_grad():
            for data, _ in val_loader:
                data = data.to(self.device)
                
                # Forward pass
                decoded_outputs, mu, logvar = model(data)
                
                # Compute loss
                loss, n_loss, c_loss, k_loss = loss_fn(decoded_outputs, data, mu, logvar, beta)
                
                # Accumulate losses
                total_loss += loss.item()
                num_loss += n_loss.item()
                cat_loss += c_loss.item()
                kl_loss += k_loss.item()
        
        # Average losses
        n_samples = len(val_loader.dataset)
        return {
            'total_loss': total_loss / n_samples,
            'num_loss': num_loss / n_samples,
            'cat_loss': cat_loss / n_samples,
            'kl_loss': kl_loss / n_samples
        }
    
    def _save_model(
        self,
        model: VAE,
        optimizer: torch.optim.Optimizer,
        training_results: Dict[str, Any],
        save_dir: str,
        strategy: str,
        beta: float
    ) -> None:
        """Save trained model and metadata."""
        checkpoint = {
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
            'categorical_cardinality': self.preprocessing_objects['categorical_cardinality'],
            'num_numerical': len(self.preprocessing_objects['num_cols']),
            'input_dim': model.input_dim,
            'hidden_dim': self.config.model.HIDDEN_DIM,
            'latent_dim': self.config.model.LATENT_DIM,
            'beta': beta,
            'strategy': strategy,
            'training_results': training_results,
            'config': {
                'batch_size': self.config.model.BATCH_SIZE,
                'learning_rate': self.config.model.LEARNING_RATE,
                'num_epochs': self.config.model.NUM_EPOCHS
            }
        }
        
        model_path = os.path.join(save_dir, 'vae_model_final.pth')
        torch.save(checkpoint, model_path)
        logger.info(f"Model saved to {model_path}")
    
    def _save_training_history(self, history: Dict[str, List], save_dir: str) -> None:
        """Save training history as CSV."""
        df = pd.DataFrame(history)
        df['epoch'] = range(1, len(df) + 1)
        df.to_csv(os.path.join(save_dir, 'training_history.csv'), index=False)
    
    def _create_training_plots(self, history: Dict[str, List], save_dir: str) -> None:
        """Create training visualization plots."""
        epochs = range(1, len(history['train_loss']) + 1)
        
        # Loss curves with beta schedule
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10))
        
        # Main losses
        ax1.plot(epochs, history['train_loss'], 'b-', label='Training Loss')
        ax1.plot(epochs, history['val_loss'], 'r-', label='Validation Loss')
        ax1.set_xlabel('Epochs')
        ax1.set_ylabel('Loss')
        ax1.set_title('Training and Validation Loss')
        ax1.legend()
        ax1.grid(True, alpha=0.3)
        
        # Beta schedule
        ax1_beta = ax1.twinx()
        ax1_beta.plot(epochs, history['beta_values'], 'g--', alpha=0.7, label='Beta')
        ax1_beta.set_ylabel('Beta Value', color='g')
        ax1_beta.tick_params(axis='y', labelcolor='g')
        
        # Component losses
        ax2.plot(epochs, history['num_loss'], 'g-', label='Numerical Loss')
        ax2.plot(epochs, history['cat_loss'], 'm-', label='Categorical Loss')
        ax2.plot(epochs, history['kl_loss'], 'c-', label='KL Loss')
        ax2.set_xlabel('Epochs')
        ax2.set_ylabel('Component Loss')
        ax2.set_title('Loss Components')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, 'training_curves.png'), dpi=300)
        plt.close()
    
    def _create_strategy_summary(self, strategy: str, strategy_results: Dict[str, Any]) -> None:
        """Create summary for a single strategy."""
        strategy_dir = os.path.join(self.config.paths.MODELS_DIR, strategy)
        
        # Extract summary data
        summary_data = []
        for beta, results in strategy_results.items():
            if 'error' not in results:
                final_metrics = results['final_metrics']
                summary_data.append({
                    'beta': beta,
                    'train_loss': final_metrics['train_loss'],
                    'val_loss': final_metrics['val_loss'],
                    'num_loss': final_metrics['num_loss'],
                    'cat_loss': final_metrics['cat_loss'],
                    'kl_loss': final_metrics['kl_loss'],
                    'training_time': results['training_time'],
                    'epochs_trained': results['epochs_trained']
                })
        
        if summary_data:
            df = pd.DataFrame(summary_data)
            df.to_csv(os.path.join(strategy_dir, 'beta_results_summary.csv'), index=False)
            
            # Create trade-off plot
            self._create_tradeoff_plot(df, strategy_dir, strategy)
    
    def _create_tradeoff_plot(self, df: pd.DataFrame, strategy_dir: str, strategy: str) -> None:
        """Create reconstruction vs KL trade-off plot."""
        plt.figure(figsize=(10, 8))
        
        reconstruction_loss = df['num_loss'] + df['cat_loss']
        
        scatter = plt.scatter(
            df['kl_loss'], reconstruction_loss, 
            s=100, c=df['beta'], cmap='viridis'
        )
        
        # Add beta labels
        for _, row in df.iterrows():
            recon = row['num_loss'] + row['cat_loss']
            plt.annotate(
                f'β={row["beta"]}',
                (row['kl_loss'], recon),
                xytext=(10, 5),
                textcoords='offset points',
                fontsize=10
            )
        
        plt.colorbar(scatter, label='Beta Value')
        plt.title(f'VAE Trade-off: Reconstruction vs KL Divergence ({strategy} annealing)')
        plt.xlabel('KL Divergence Loss')
        plt.ylabel('Reconstruction Loss (Numerical + Categorical)')
        plt.grid(True, alpha=0.3)
        plt.savefig(os.path.join(strategy_dir, 'vae_tradeoff.png'), dpi=300)
        plt.close()
    
    def _create_cross_strategy_comparisons(self, all_results: Dict[str, Dict[str, Any]]) -> None:
        """Create comparisons across different strategies."""
        # Implementation for cross-strategy comparison plots
        # This would create plots comparing different strategies
        pass